...
*how* the project was built, STEP by STEP. See `Code/mine-flow-docs/METHOD.md` for the
...
**history** (never rewritten); the docs hub (`Code/mine-flow-docs/`) is **state** (kept
...
- **Filenames** are preserved from when they were written:
  - PLAN: `mine-flow-STEP-N-PLAN.md`
  - Substep prompt: `mine-flow-STEP-N.M-PROMPT.md` (fractional substeps like `5a` are fine)
- Each phase folder keeps a `README.md` summary table (from
  `Code/mine-flow-docs/templates/phase-readme-template.md`), updated by hand as STEPs complete.
...
(`Code/mine-flow-docs/templates/planning-session.md`, *"run the planning session"*): it
...
`Code/mine-flow-docs/runbooks/check-in.md` (reconcile docs vs. code both ways, re-check
conditional-session coverage, review accepted risks/debt in
`Code/mine-flow-docs/registries/risks.yml`, and run the full test suite). Its completed
report is written under `Code/mine-flow-docs/reports/`; the archived STEP folder here keeps
...
> questions from `Code/mine-flow-docs/BOOTSTRAP-PROMPT.md` Stage 0, create/update the file,
> and continue.
...
1. **Reserve the number in the index.** Look up the STEP in `prompts/STEP-index.md`. If it's
   ...
   **before** branching or writing. If the push is rejected, someone reserved concurrently —
   ...
   See `Code/mine-flow-docs/runbooks/collaboration.md`.
...
3. **Plan the tests before finalizing the substeps.** Read the Test Strategy architecture doc
   (`Code/mine-flow-docs/architecture/*-test-strategy.md`) and decide which test tiers this
...
4. **Write the PLAN** from `Code/mine-flow-docs/templates/step-plan-template.md`: motivation,
...
5. **Write the substep prompts** from `Code/mine-flow-docs/templates/substep-prompt-template.md` —
   ...
   *(For the architecture STEP-1, the substeps are the interview sessions in
   `Code/mine-flow-docs/templates/architecture-sessions/` — you don't author those from
   ...
   `Code/mine-flow-docs/runbooks/check-in.md`; the PLAN just points there. An **Incident
   STEP** is the same kind of thin STEP — its three substeps (RCA → find similar → fix) are
   defined in `Code/mine-flow-docs/runbooks/incident-postmortem.md`, opened by that runbook
   when responding to a production incident; its durable postmortem report starts from
   `Code/mine-flow-docs/templates/reports/incidents/incident-postmortem-report-template.md`
   and is saved under `Code/mine-flow-docs/reports/incidents/`. A **late conditional-session
...
6. **On completion:** run the STEP's review — your team's standard **PR / code review** (the
   ...
   next-action resolver (`Code/mine-flow-docs/METHOD.md` §10).
...
> rows** and never re-sort. See `Code/mine-flow-docs/runbooks/collaboration.md`.
