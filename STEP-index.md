...
| STEP-47 | Android Build Chain Remediation (AGP 9 / local device builds) | | Planned | `mine-flow-app`, `mine-flow-docs` | Restore a working local Android build so device E2E can run at all. Root cause: under AGP 9.1.0 the `flutter` Gradle extension is not populated in plugin subprojects, so `package_info_plus`'s `compileSdk = flutter.compileSdkVersion` resolves null, and the `android.newDsl=false` / `builtInKotlin` opt-outs in `gradle.properties` are ignored. Decide between migrating to AGP-9 built-in Kotlin, downgrading AGP, or upgrading the offending plugins; document host prerequisites (PUB_CACHE on the SDK drive, JDK 17). **Blocks STEP-48's Android surface.** |
