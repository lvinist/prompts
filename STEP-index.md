...
| STEP-45 | Release-Candidate E2E & Runtime Design Review     | Antigravity | Done | `mine-flow-app`, `mine-flow-docs`, `prompts`   | Execute critical Android and web journeys against staging, including field-critical offline/sync behavior, and capture the required runtime Impeccable design-review evidence for responsive, accessible, localized UI states. Resolve or explicitly carry forward findings before considering a production release. **Delivered:** `integration_test` harness, dual-platform CI gate, 15 journey tests (analyze 0 / format clean), ADR-0017, Doc 12 v1.1, Doc 09 v0.3.0. **NOT delivered — runtime evidence deferred to STEP-48:** 14/15 journeys never executed (no staging credentials), all design-review items Unverified; only NR-001 resolved, NR-002..006 carried forward as RISK-0015..0019. See the corrected substep table below. |
...
| STEP-48 | Runtime Evidence — resolve STEP-45's carried-forward findings | Gemini 3.7 Flash High (48.0, 48.6, 48.9, 48.21, 48.24) · Gemini 3.1 Pro High (48.2–48.5, 48.7, 48.8, 48.11–48.14, 48.18–48.20, 48.28) · GPT 5.6 Terra (48.16, 48.17, 48.26, 48.29) · GPT 5.6 Luna (48.22, 48.30) · Hermes/Claude Opus 4.8 (48.1, 48.10, 48.15, 48.23, 48.25, 48.27) | In progress | `mine-flow-app`, `mine-flow-docs`, `prompts` | Broad runtime evidence exists, but close is blocked: local gates pass (`flutter test` 448), while branch-head CI run [`33327930159`](https://github.com/lvinist/mine-flow-app/actions/runs/33327930159) executed real journeys and failed both E2E jobs (`14 passed, 10 failed, 2 skipped` on Android) due to staging schema/fixture and runtime blockers. 48.15 recorded the honest Deferred close; Phase 4 must not open until a branch-head run has all four required jobs green with non-zero executed counts. **Remediation wave 48.16–48.30 added 2026-08-31–2026-09-06** (PLAN decisions D6/D7): triage gate, `timeline_milestones` + `benchmarks` migrations applied to staging with regenerated contract types and Doc 04 entity rows, datasource/write-path column reconciliation, seed and fixture alignment to `defaultSiteId`, finder repair, UI/harness fixes (timeline overflow, Material ancestor, `UploadFilePage` guard, Android screenshot leg), persistence/offline-integrity diagnosis, local full-gate re-run, `mine-flow-app` git object-store repair & wave commit (48.25), fresh branch-head CI gate (48.26), post-gate schema/reporting/synchronization contract remediation (48.27), and audit substeps 48.28–48.30: registry/doc truth repair, evidence-guard hardening, and dead-affordance/shadow-file cleanup. 48.15 re-runs only on a GO verdict from 48.26. |
| STEP-49 | Throughstone Template Hardening (process feedback) | | Planned | Throughstone template repo (to be located) | Feed this project's failure modes back into the scaffold: a mechanical Unverified-vs-Done honesty gate at STEP close, phantom-close detection against disk, a credential/host-toolchain pre-flight substep for runtime STEPs, guidance against accumulating a multi-repo uncommitted close, and the dirty-index/trunk-reservation recovery recipe. No mine-flow application code. |
| STEP-50 | Phase 3 Check-in (post-STEP-47 dependency overhaul) | Hermes (Claude Opus 4.8) | In progress | `mine-flow-docs`, `prompts` | Run `runbooks/check-in.md` at the STEP-47 breakpoint, ~10 STEPs after the STEP-39/40 check-in: mechanical `scripts/check.sh` pass, doc-drift reconciliation in both directions across the architecture docs, conditional-session coverage re-evaluation, repo-README / interface-contract / docstring sweep, `registries/risks.yml` review (RISK-0001..0020), the S0/S1/S2 security-review gate, and the full `flutter test` suite. Review-and-verification only — no application code. Sequenced before STEP-48 so its runtime-evidence work starts from reconciled docs. |
...
> obtained). **`Deferred` here means the deliverable exists and is committed but has never been
> executed against a real backend/device — the runtime evidence is deferred to STEP-48**, which
> owns the real verification. (`Deferred` is used rather than a free-text "Unverified" so
> `scripts/check.sh` status validation still passes; the per-row detail states exactly what is
> and is not verified.)
...
| 45.3 | Auth & session E2E journey | Deferred | **Unverified** — `auth_journey_test.dart` authored & committed; never run against staging (no credentials) → STEP-48 |
| 45.4 | Attendance & Daily Logging E2E | Deferred | **Unverified** — `attendance_journey_test.dart`, `daily_log_journey_test.dart` authored; not run → STEP-48 |
| 45.5 | Cut/Fill & Land Clearing E2E | Deferred | **Unverified** — `cut_fill_journey_test.dart`, `land_clearing_journey_test.dart` authored; not run → STEP-48 |
| 45.6 | Inventory & Equipment Checks E2E | Deferred | **Unverified** — `inventory_journey_test.dart`, `equipment_check_journey_test.dart` authored; not run → STEP-48 |
| 45.7 | Benchmark E2E + route-registration (NR-006) | Deferred | **Unverified** — `benchmark_journey_test.dart` authored; NR-006 not settled → RISK-0019 → STEP-48 |
| 45.8 | Data Bucket E2E + real Drive upload (NR-004/005) | Deferred | **Unverified** — `data_bucket_journey_test.dart` authored; no Drive service-account creds → RISK-0017, RISK-0018 (stay deferred until creds exist) |
| 45.9 | Reporting / PDF E2E (NR-001) | Deferred | **Partial: code Done, E2E Unverified.** **NR-001 genuinely resolved** (config controls locked during generation — real code change); `reporting_journey_test.dart` authored but not run → STEP-48 |
| 45.10 | Timeline & Notifications E2E | Deferred | **Unverified** — `timeline_journey_test.dart`, `notifications_journey_test.dart` authored; not run → STEP-48 |
| 45.11 | Field-critical offline/sync full journey | Deferred | **Partial: sync contract verified, staging E2E Unverified.** `offline_sync_journey_test.dart` Part A (staging) Unverified; **Part B verifies the SyncQueueManager contract unconditionally on-device** (offline defer, relaunch persistence, FIFO drain, retry→fail at maxRetries, last-write-wins). Q5 conflict method documented. Blocked locally by absent creds + broken local Android build (→ STEP-47) + web integration tests unsupported |
| 45.12 | Live RLS / authorization E2E against staging | Deferred | **Unverified** — `rls_authorization_journey_test.dart` authored; no per-role staging accounts; STEP-44 S0 deferred row updated to Unverified → STEP-48 (partial: single test user only) |
| 45.13 | go_router v17 deep-link validation | Deferred | **Unverified** — `deep_link_journey_test.dart` authored; RISK-0006 remains `open` (E2E unverified) → STEP-48 |
| 45.14 | Runtime Impeccable design review | Deferred | **Unverified** — `reports/2026-08-27-step-0045-runtime-design-review.md` records every item (responsive, themes, localization, a11y, NR-002, NR-003, RISK-0011) as Unverified; no screenshots captured; RISK-0015, RISK-0016 raised → STEP-48 |
